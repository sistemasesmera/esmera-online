---
title: "Uñas de Gel"
internalCode: "UGE1"
shortDescription: "Aprende a aplicar, modelar y mantener uñas de gel: desde la preparación natural hasta el curado en lámpara y el acabado profesional."
categories: ["unas"]
level: "Iniciación"
duration: "20 horas"
modality: "Online"
tutor: "equipo-esmera"
coverImage: "./unas-gel.png"
objectives:
  - text: "Preparar la uña natural correctamente antes de la aplicación del gel."
    icon: "sparkles"
  - text: "Aplicar y modelar el gel para conseguir una forma natural y duradera."
    icon: "target"
  - text: "Curar el gel en lámpara UV/LED controlando tiempos y capas."
    icon: "briefcase"
  - text: "Realizar el mantenimiento y relleno de uñas de gel con técnica profesional."
    icon: "refresh"
contents: []
hasFinalProject: false
certificate: "Diploma privado"
faq: []
relatedCourses:
  - "unas-acrilicas"
  - "unas-acrigel"
price: 49
stripePaymentLink: "https://buy.stripe.com/3cI6oJ50ifsogiFc2I28803"
featured: false
order: 14
---

Aprende a trabajar con gel para uñas de forma profesional: preparación, aplicación, modelado y curado en lámpara, hasta conseguir un resultado duradero y de calidad. Incluye técnicas de relleno y mantenimiento.
