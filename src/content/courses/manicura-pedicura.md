---
title: "Manicura y Pedicura Profesional"
internalCode: "CMP1"
shortDescription: "Domina las técnicas profesionales de manicura y pedicura: preparación, tratamiento de cutículas, esmaltado y cuidado completo de manos y pies."
categories: ["unas"]
level: "Iniciación"
duration: "90 horas"
modality: "Online"
tutor: "equipo-esmera"
coverImage: "./manicura-y-pedicura.png"
objectives:
  - text: "Aplicar los protocolos de preparación e higiene en manicura y pedicura profesional."
    icon: "shield"
  - text: "Trabajar cutículas, lima y esmaltado con técnica y precisión."
    icon: "sparkles"
  - text: "Realizar tratamientos de hidratación y cuidado de manos y pies."
    icon: "award"
  - text: "Atender al cliente con profesionalidad y ofrecer un servicio de calidad."
    icon: "users"
contents: []
hasFinalProject: false
certificate: "Diploma privado"
faq: []
relatedCourses:
  - "unas-esmera"
  - "decoracion-unas-basica"
price: 349
featured: false
order: 12
---

Formación profesional en manicura y pedicura que te prepara para ofrecer un servicio completo y de calidad: desde la preparación e higiene hasta el tratamiento de cutículas, esmaltado y cuidado de manos y pies.
